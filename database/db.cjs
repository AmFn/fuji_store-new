...
class PhotoDatabase {
  ...
  #prepareStatements() {
    this.stmts = {
      ...
      upsertPhoto: this.db.prepare(`
        INSERT INTO photos (
          path, hash, size, width, height, created_at, updated_at,
      ...
      getPhotosPage: this.db.prepare(`
        SELECT id, path, hash, size, width, height, created_at, updated_at,
  ...
  getDateGroups() {
    const photos = this.db.prepare(`
      SELECT 
        id, path, hash, thumbnail_status, created_at, metadata_json,
    ...
    photos.forEach(photo => {
      ...
      try {
        ...
        const dateTime = metadata.dateTime || new Date(photo.created_at).toISOString();
        const date = new Date(dateTime);
        dateStr = date.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
      } catch (e) {
        dateStr = new Date(photo.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
      }
  ...
  getPhotosByDate(dateStr, limit = 0, preferThumbnail = true) {
    ...
    const matchedPhotos = allPhotos.filter(photo => {
      try {
        ...
        const photoDate = new Date(photo.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        return photoDate === dateStr;
      }
    });
